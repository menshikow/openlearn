...
```bash
# Kopiere OpenLearn in dein Projekt
git clone https://github.com/menshikow/openlearn.git
cp -r openlearn/.opencode/ ./dein-projekt/

# Starte opencode und führe aus
/openlearn-init
```

## Schnellstart

1. `/openlearn-init` - Projekt einrichten
2. `/openlearn-feature` - Funktion planen
3. `/openlearn-guide` - Anleitung beim Coden erhalten
4. `/openlearn-done` - 6 Qualitätsgatter bestehen
...
| `/openlearn-feature` | Funktion planen |
| `/openlearn-guide` | Anleitung erhalten |
| `/openlearn-stuck` | Debuggen mit Protokoll D |
| `/openlearn-done` | Mit Gattern abschließen |
| `/openlearn-test` | Test-Anleitung |
| `/openlearn-retro` | Gelerntes festhalten |
